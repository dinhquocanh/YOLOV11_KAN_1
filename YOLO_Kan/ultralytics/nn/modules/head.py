...
from ultralytics.nn.modules.kan import KANLayer
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # --- TẠO KANLayer tương ứng với từng input channel của x[i] ---
        # Giả sử bạn muốn xử lý từng channel đầu vào x[i] riêng biệt,
        # ở đây mình tạo 1 danh sách ModuleList chứa 1 KANLayer mỗi input channel
        self.kan_layers = nn.ModuleList()
        for c_in in ch:
            # Tạo 1 list KANLayer cho từng channel input của layer i
            kan_per_channel = nn.ModuleList([KANLayer(in_dim=1, out_dim=1) for _ in range(c_in)])
            self.kan_layers.append(kan_per_channel)

    def forward(self, x):
        if self.end2end:
            return self.forward_end2end(x)
        
        # Áp dụng KANLayer cho từng feature map trong list x
        for i in range(self.nl):
            xi = x[i]  # shape (B, C, H, W)
            B, C, H, W = xi.shape

            kan_per_channel = self.kan_layers[i]
            # Chạy KANLayer từng channel một
            kan_out_channels = []
            for c_idx in range(C):
                # Lấy channel c_idx: shape (B, H, W)
                x_c = xi[:, c_idx, :, :]  # (B, H, W)
                x_c_flat = x_c.reshape(-1, 1)  # reshape để feed KANLayer (B*H*W, 1)
                y_c, _, _, _ = kan_per_channel[c_idx](x_c_flat)  # output (B*H*W, 1)
                y_c = y_c.reshape(B, H, W)  # reshape lại (B, H, W)
                kan_out_channels.append(y_c)
            
            # stack lại (B, C, H, W)
            kan_out = torch.stack(kan_out_channels, dim=1)

            # Cộng kết quả KANLayer vào input feature map
            x[i] = xi + kan_out

            # Giữ nguyên phần concat cv2 + cv3 như cũ
